...
export function EmailInputNode({ errors, setFocus, register, inputActiveState, setInputActiveState, stopState, setStopState, adaptiveErrorMessageState, shrinkedState}){
    ...
    return(
        <>
            <div className={styles.emailFlexWrapper}>
                <label htmlFor={"a-p-email"} style={(shrinkedState)? {display:"none"} : {display:"block"}} className={styles.emailLabel}>Email address:</label>
                <div className={(errors?.email)? styles.notValid + " " + styles.active + " " + styles.inputWrapper : (inputActiveState) ? styles.active + " " + styles.inputWrapper : styles.inputWrapper}>
                    ...
                    <label htmlFor={"a-p-email"} style={(shrinkedState)? {display:"block"} : {display:"none"}} className={(errors?.email)?  styles.notValid + " " + styles.emailLabel : styles.emailLabel }>Email address:</label>
                    <label htmlFor={"a-p-email"} style={(shrinkedState)? {display:"none"} : {display:"block"}} className={(errors?.email)? styles.notValid + " " + styles.floatingDesc : styles.floatingDesc}>Email</label>
                </div>
